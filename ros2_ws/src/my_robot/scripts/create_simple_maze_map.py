...
tree = ET.parse(os.path.join(PACKAGE_DIR, 'worlds', 'simple_maze.world'))
root = tree.getroot()

# Map parameters for 25x25m maze
resolution = 0.05
map_width_px = 800  # Covers x: 0 to 40m (entrance cage + maze + exit cage)
map_height_px = 600  # Covers y: -15 to 15m

# Create white image
img = Image.new('L', (map_width_px, map_height_px), 254)
draw = ImageDraw.Draw(img)

# Origin offset - maze starts at x=4.0, centered at y=0
origin_x = 0.0
origin_y = -15.0
...
img.save(os.path.join(PACKAGE_DIR, 'maps', 'world_reference.png'))
print(f"Simple maze map created: {map_width_px}x{map_height_px} - saved as world_reference.png")
